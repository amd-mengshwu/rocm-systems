...
public:
  ...
  void invalidate(uint64_t addr, uint32_t vmid = 0) {
    ...
    if (cached_read_line_addr_ == CacheStore::line_address(addr) && cached_read_line_vmid_ == vmid)
...
private:
  ...
  uint32_t cached_read_line_vmid_ = 0;
