...
hipFileGetStatsL1(hipFileStatsLevel1_t *stats)
{
    ...
    *stats = {};
    ...
    if (stats->read_lat_sum_us > 0) {
        stats->read_bw_bytes_per_sec = static_cast<uint64_t>(static_cast<double>(stats->read_bytes) * 1e6 /
                                                             static_cast<double>(stats->read_lat_sum_us));
    }
    if (stats->write_lat_sum_us > 0) {
        stats->write_bw_bytes_per_sec = static_cast<uint64_t>(static_cast<double>(stats->write_bytes) * 1e6 /
                                                              static_cast<double>(stats->write_lat_sum_us));
    }
